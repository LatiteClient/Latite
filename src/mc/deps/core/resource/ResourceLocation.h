...
namespace SDK {
	namespace Core {
		...
		public:
			...
			PathBuffer const* operator->() const {
				return this;
			}
		};
	}
	...
	public:
		...
		explicit ResourceLocation(ResourceFileSystem fileSystem) : mFileSystem(fileSystem) {
		}

		ResourceLocation(std::string const& path, ResourceFileSystem fileSystem) : mFileSystem(fileSystem) {
			mPath->value = path;
		}
