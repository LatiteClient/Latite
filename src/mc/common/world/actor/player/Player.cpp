...
void SDK::Player::displayClientMessage(std::string const& message) {
	if (internalVers >= V1_21_20) {
		...
		memory::callVirtual<void>(this, mvGetOffset<0xC9, 0xC8, 0xC8, 0xC8, 0xC8, 0xC8, 0xCD, 0xCE, 0xD2, 0xD2, 0x0, 0x0, 0x0, 0x0, 0x0>(), message, opt);
		return;
	}
...
std::string SDK::Player::getXUID() {
	return memory::callVirtual<std::string>(this, SDK::mvGetOffset<0xEA, 0xEC, 0xEC, 0xEC, 0xEC, 0xEC, 0xEC, 0xF1, 0xF2, 0xF3, 0xF3, 0xF6, 0x18C, 0x1B6, 0x1BC>());
}
