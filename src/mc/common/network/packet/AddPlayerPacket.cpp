...
bool SDK::AddPlayerPacket::tryGetNameTag(uint64_t* runtimeId, std::string* nameTag) const {
	if (!runtimeId || !nameTag) return false;

	constexpr uint16_t nameTagMetadataId = 4;
	constexpr uint8_t stringMetadataType = 4;
	constexpr uintptr_t runtimeIdOffset = 0x80;
	constexpr uintptr_t metadataVectorOffset = 0xD0;

	auto packetBase = reinterpret_cast<uintptr_t>(this);
	const auto packetRuntimeId = *reinterpret_cast<const uint64_t*>(packetBase + runtimeIdOffset);
	auto* metadataBegin = *reinterpret_cast<void***>(packetBase + metadataVectorOffset);
	auto* metadataEnd = *reinterpret_cast<void***>(packetBase + metadataVectorOffset + sizeof(void*));
