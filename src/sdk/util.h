...
#define MV_DETAIL_GETOFFSET(offs1, offs2) \
    sdk::internalVers == sdk::VLATEST ? offs1 : offs2

#define MVCLASS_FIELD(type, name, offset1, offset2) __declspec(property(get = __get_field_##name, put = __set_field_##name)) type name;                             \
type& __get_field_##name() const { return util::directAccess<type>(this, MV_DETAIL_GETOFFSET(offset1, offset2)); }                                    \
template<typename T> void __set_field_##name(const T& value) { util::directAccess<type>(this, (MV_DETAIL_GETOFFSET(offset1, offset2))) = value; }
