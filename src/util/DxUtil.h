...
namespace d2d {
    ...
    public:
        ...
        constexpr Color(D2D1_COLOR_F col)
        ...
        constexpr Color operator+(Color right) { return Color(r + right.r, g + right.g, b + right.b, a); }

        constexpr Color operator+(float right) { return Color(r + right, g + right, b + right, a); }

        constexpr Color operator*(Color right) { return Color(r * right.r, g * right.g, b * right.b, a); }

        constexpr Color operator*(float right) { return Color(r * right, g * right, b * right, a * right); }
        ...
        [[nodiscard]] bool isInvalid() const {
            return r < 0.f || g < 0.f || b < 0.f || isnan(r) || isnan(g) || isnan(b) || isinf(r) || isinf(g) ||
                   isinf(b);
        }
        ...
        [[nodiscard]] constexpr Color asAlpha(float al) const { return { r, g, b, al }; }

        [[nodiscard]] constexpr D2D1_COLOR_F get() const { return D2D1_COLOR_F(r, g, b, a); }
    ...
    public:
        ...
        [[nodiscard]] D2D1_RECT_F get() const { return D2D1::RectF(left, top, right, bottom); }

        [[nodiscard]] Vec2 getPos() const { return { left, top }; }
        ...
        [[nodiscard]] Vec2 getSize() const { return { right - left, bottom - top }; }
        ...
        [[nodiscard]] Vec2 center(Vec2 otherSize = { 0, 0 }) const {
            return Vec2(left + ((right - left) / 2 - (otherSize.x / 2)),
        ...
        [[nodiscard]] Vec2 centerYL() const { return Vec2(left, top + ((bottom - top) / 2)); }

        // Center Y-axis, X is right
        [[nodiscard]] Vec2 centerYR() const { return Vec2(right, top + ((bottom - top) / 2)); }

        // Center X-axis, Y is top
        [[nodiscard]] Vec2 centerXT() const { return Vec2(left + ((right - left) / 2), top); }

        // Center X-axis, Y is bottom
        [[nodiscard]] Vec2 centerXB() const { return Vec2(left + ((right - left) / 2), bottom); }
        ...
        operator D2D1_RECT_F() { return { left, top, right, bottom }; }
    };

    [[nodiscard]] inline Rect rectFromStart(Rect const& bounds, float startOffset, float top, float width, float height,
                                            bool rtl) {
        if (rtl) {
    ...
    [[nodiscard]] inline Rect rectFromEnd(Rect const& bounds, float endOffset, float top, float width, float height,
                                          bool rtl) {
        if (rtl) {
    ...
    [[nodiscard]] inline Rect controlAtStart(Vec2 const& pos, float areaWidth, float controlWidth, float height,
                                             bool rtl) {
        if (rtl) {
    ...
    [[nodiscard]] inline Rect labelAfterStartControl(Rect const& controlRect, Vec2 const& pos, float areaWidth,
                                                     float gap, bool rtl) {
        if (rtl) {
