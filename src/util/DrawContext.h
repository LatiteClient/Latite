...
public:
    ...
    virtual void drawRoundedRectangle(RectF, d2d::Color const& color, float radius = 10.f, float lineThickness = 1.f,
                                      OutlinePosition outPos = OutlinePosition::Center) = 0;

    /// Draws text. Be careful when setting cache to true
    virtual void drawText(RectF const& rc, std::wstring const& text, d2d::Color const& color,
                          Renderer::FontSelection font, float size = 30.f,
                          DWRITE_TEXT_ALIGNMENT alignment = DWRITE_TEXT_ALIGNMENT_LEADING,
                          DWRITE_PARAGRAPH_ALIGNMENT verticalAlign = DWRITE_PARAGRAPH_ALIGNMENT_NEAR, bool cache = true,
                          bool hyphen = false) = 0;
    /// Be careful when setting cache to true
    virtual Vec2 getTextSize(std::wstring const& text, Renderer::FontSelection font, float size,
                             bool trailingWhitespace = true, bool cache = true,
                             std::optional<Vec2> bounds = std::nullopt) = 0;
    /// Be careful when setting cache to true
    virtual RectF getTextRect(std::wstring const& text, Renderer::FontSelection font, float size, float pad = 0.f,
                              bool cache = true) = 0;
...
public:
    ...
    virtual void drawRoundedRectangle(RectF, ID2D1Brush*, float radius = 10.f, float lineThickness = 1.f,
    ...
    virtual void drawRoundedRectangle(RectF, d2d::Color const& color, float radius = 10.f, float lineThickness = 1.f,
                                      OutlinePosition outPos = OutlinePosition::Center) override;
    virtual void drawText(RectF const& rc, std::wstring const& text, d2d::Color const& color,
                          Renderer::FontSelection font, float size = 30.f,
                          DWRITE_TEXT_ALIGNMENT alignment = DWRITE_TEXT_ALIGNMENT_LEADING,
                          DWRITE_PARAGRAPH_ALIGNMENT verticalAlign = DWRITE_PARAGRAPH_ALIGNMENT_NEAR, bool cache = true,
                          bool hyphen = false) override;
    virtual Vec2 getTextSize(std::wstring const& text, Renderer::FontSelection font, float size,
                             bool trailingWhitespace = true, bool cache = true,
                             std::optional<Vec2> bounds = std::nullopt) override;
    virtual RectF getTextRect(std::wstring const& text, Renderer::FontSelection font, float size, float pad = 0.f,
                              bool cache = true) override;

    void drawTextClipped(RectF const& clipRect, RectF const& textRect, std::wstring const& text,
    ...
    void drawWrappedTextClipped(RectF const& textRect, std::wstring const& text, d2d::Color const& color,
                                Renderer::FontSelection font, float size,
                                DWRITE_TEXT_ALIGNMENT alignment = DWRITE_TEXT_ALIGNMENT_LEADING,
                                DWRITE_PARAGRAPH_ALIGNMENT verticalAlignment = DWRITE_PARAGRAPH_ALIGNMENT_NEAR,
                                bool cache = false);
    void drawSingleLineFitted(RectF const& textRect, std::wstring const& text, d2d::Color const& color,
                              Renderer::FontSelection font, float size,
                              DWRITE_TEXT_ALIGNMENT alignment = DWRITE_TEXT_ALIGNMENT_LEADING,
                              DWRITE_PARAGRAPH_ALIGNMENT verticalAlignment = DWRITE_PARAGRAPH_ALIGNMENT_CENTER,
                              bool cache = false);
    [[nodiscard]] float getTextLineHeight(Renderer::FontSelection font, float size);
    [[nodiscard]] float getMeasuredTextHeight(RectF const& textRect, std::wstring const& text,
                                              Renderer::FontSelection font, float size, float maxLines = 0.f);
    [[nodiscard]] std::wstring ellipsizeToWidth(std::wstring text, Renderer::FontSelection font, float size,
                                                float maxWidth);
};
...
public:
    ...
    void drawImage(SDK::TexturePtr& texture, Vec2 const& pos, Vec2 const& size, Vec2 const& uvPos, Vec2 const& uvSize,
                   d2d::Color const& flushCol);
    d2d::Rect drawItem(SDK::ItemStack* item, Vec2 const& pos, float sizeModifier = 1.f, float opacity = 1.f);
    bool drawActor(SDK::Actor* actor, d2d::Rect const& bounds, float opacity = 1.f);
