...
void util::doThrowIfFailed(HRESULT hr, int line, std::string func) {
    if (FAILED(hr)) {
        ...
        MessageBoxA(
            NULL,
            (std::string("An error has occured! Please screenshot and report this to the developer!\n\n") + ss.str())
                .c_str(),
            "DX assertion failed", MB_ICONERROR | MB_OK);
#endif
...
std::string d2d::Color::getHex() const {
    ...
    return std::format("{:02X}{:02X}{:02X}", static_cast<int>(std::round(r * 255.f)),
                       static_cast<int>(std::round(g * 255.f)), static_cast<int>(std::round(b * 255.f)));
}
