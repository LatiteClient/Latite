...
void DXContext::drawText(RectF const& rc, std::string const& text, d2d::Color const& color, float size, DWRITE_TEXT_ALIGNMENT alignment)  {
	brush->SetColor(color.get());
	auto ws = util::strToWstr(text);
	IDWriteTextLayout* layout;
	this->factory->CreateTextLayout(ws.c_str(), static_cast<uint32_t>(ws.size()),
