...
JsValueRef Filesystem::write(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                             void* callbackState) {
    auto undef = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 4)) return undef;
    if (!Chakra::VerifyParameters(
            { { arguments[1], JsString }, { arguments[2], JsTypedArray }, { arguments[3], JsFunction } }))
        return undef;
...
JsValueRef Filesystem::read(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                            void* callbackState) {
    auto undef = Chakra::GetUndefined();
...
JsValueRef Filesystem::writeSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                                 void* callbackState) {
    auto undef = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 3)) return undef;
...
JsValueRef Filesystem::readSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                                void* callbackState) {
...
JsValueRef Filesystem::existsSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                                  void* callbackState) {
    auto ret = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 2)) return ret;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString } })) return ret;
...
JsValueRef Filesystem::createDirectorySync(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef Filesystem::appendSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                                  void* callbackState) {
    auto undef = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 3)) return undef;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString }, { arguments[2], JsTypedArray } })) return undef;
...
JsValueRef Filesystem::deleteFile(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                                  void* callbackState) {
    auto ret = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 2)) return ret;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString } })) return ret;
...
JsValueRef Filesystem::readdirSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                   unsigned short argCount, void* callbackState) {
    auto ret = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 2)) return ret;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString } })) return ret;
...
JsValueRef Filesystem::moveSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                                void* callbackState) {
    auto ret = Chakra::GetUndefined();
...
void Filesystem::FSAsyncOperation::getArgs() {
    ...
    if (data.has_value() && outData) {
        ...
        JS::JsCreateTypedArray(JsArrayTypeUint8, JS_INVALID_REFERENCE, 0, static_cast<unsigned int>(data->size()),
                               &jData);
        BYTE* storage;
        unsigned int bufLen;
        JsTypedArrayType at;
