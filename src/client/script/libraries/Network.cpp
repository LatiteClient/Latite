...
JsValueRef Network::get(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                        void* callbackState) {
    // TODO: Does not support contentType or custom headers
    // This whole function and NetAsyncOperation should probably be rewritten to not use threads, because we don't even
    // need another thread to do async network operations in the first place.

    auto ret = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 4)) return ret;
    if (!Chakra::VerifyParameters(
            { { arguments[1], JsString }, { arguments[2], JsObject }, { arguments[3], JsFunction } }))
        return ret;
...
JsValueRef Network::getSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                            void* callbackState) {
    auto ret = Chakra::GetUndefined();
    if (!Chakra::VerifyArgCount(argCount, 2, true, true)) return ret;
    if (!Chakra::VerifyParameters(
            { { arguments[1], JsString }, { Chakra::TryGet(arguments, argCount, 2), JsObject, true } }))
        return ret;
    auto ws = Chakra::GetString(arguments[1]);
...
JsValueRef Network::post(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                         void* callbackState) {
...
JsValueRef Network::postSync(JsValueRef callee, bool isConstructor, JsValueRef* arguments, unsigned short argCount,
                             void* callbackState) {
    auto ret = Chakra::GetUndefined();
