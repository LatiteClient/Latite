...
JsValueRef JsCommandClass::jsConstructor(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
    ...
    auto jsc =
        new JsCommand(arguments[0], util::WStrToStr(Chakra::GetString(arguments[1])), Chakra::GetString(arguments[2]),
                      util::WStrToStr(Chakra::GetString(arguments[3])), aliases);
    return thi->construct(jsc, false);
}

JsValueRef JsCommandClass::setOnEvent(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                      unsigned short argCount, void* callbackState) {
