...
private:
    static JsValueRef CALLBACK jsConstructor(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
        ...
        if (!Chakra::VerifyParameters({ { arguments[1], JsNumber }, { arguments[2], JsNumber } }))
            return JS_INVALID_REFERENCE;

        return thi->construct(Vec3((float)Chakra::GetNumber(arguments[1]), (float)Chakra::GetNumber(arguments[2]),
    ...
    static JsValueRef CALLBACK toStringCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
        ...
        std::string add =
            std::format("{} (x={:.2f}, y={:.2f}, z={:.2f})", util::WStrToStr(thi->name), vec3.x, vec3.y, vec3.z);
...
public:
    ...
    JsVec3(class JsScript* owner)
        : JsClass(owner, class_name) {
