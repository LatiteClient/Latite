...
protected:
    ...
    static JsValueRef CALLBACK isActiveCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
        return Get(arguments[0])->isActive() ? Chakra::GetTrue() : Chakra::GetFalse();
    }

    static JsValueRef CALLBACK getKeyCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
        return Chakra::MakeInt(Get(arguments[0])->key);
    }

    static JsValueRef CALLBACK setKeyCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
    ...
    static JsValueRef CALLBACK isHoverCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
        if (!Chakra::VerifyArgCount(argCount, 2, true, true)) return JS_INVALID_REFERENCE;
        if (!Chakra::VerifyParameters(
                { { arguments[1], JsObject }, { Chakra::TryGet(arguments, argCount, 2), JsObject } }))
            return JS_INVALID_REFERENCE;
...
public:
    ...
    JsScreenClass(class JsScript* owner)
