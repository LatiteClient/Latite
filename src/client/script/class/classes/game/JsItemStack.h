...
protected:
    static JsValueRef CALLBACK toStringCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
        ...
        std::string add = std::format("{} (name={}, aux={})", util::WStrToStr(thi->name),
                                      item->item ? item->getHoverName() : "air", item->aux);
        return Chakra::MakeString(L"[object " + util::StrToWStr(add) + L"]");
    }

    static JsValueRef CALLBACK getDisplayNameCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                      unsigned short argCount, void* callbackState) {
        auto thi = reinterpret_cast<JsItemStack*>(callbackState);
        auto item = Get(arguments[0]);
    ...
    static JsValueRef CALLBACK getCountCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
        auto thi = reinterpret_cast<JsItemStack*>(callbackState);
        auto item = Get(arguments[0]);
    ...
    static JsValueRef CALLBACK getPickupTime(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                             unsigned short argCount, void* callbackState) {
        auto thi = reinterpret_cast<JsItemStack*>(callbackState);
        auto item = Get(arguments[0]);

        // double because of Y2K38 problem
        return Chakra::MakeDouble(static_cast<double>(
            std::chrono::duration_cast<std::chrono::milliseconds>(item->pickupTime.time_since_epoch()).count()));
    }

public:
    inline static const wchar_t* class_name = L"ItemStack";
    JsItemStack(class JsScript* owner)
