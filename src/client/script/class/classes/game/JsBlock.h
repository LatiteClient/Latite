...
protected:
    static JsValueRef CALLBACK toStringCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
        ...
        if (block->legacyBlock) {
            id = block->legacyBlock->namespacedId.getString();
        }
...
public:
    ...
    JsValueRef construct(SDK::Block* block, bool del) {
        ...
        if (block->legacyBlock) {
            Chakra::SetPropertyString(obj, L"name", util::StrToWStr(block->legacyBlock->namespacedId.getString()));
    ...
    void prepareFunctions() override { Chakra::DefineFunc(prototype, toStringCallback, L"toString", this); };
};
