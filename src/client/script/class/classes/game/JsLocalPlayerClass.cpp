...
JsValueRef JsLocalPlayerClass::getBreakProgress(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef JsLocalPlayerClass::getLastBreakProgress(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                    unsigned short argCount, void* callbackState) {
    auto lp = SDK::ClientInstance::get()->getLocalPlayer();
    if (!lp) {
        Chakra::ThrowError(L"Invalid local player");
...
JsValueRef JsLocalPlayerClass::getLookingAt(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    auto lp = SDK::ClientInstance::get()->getLocalPlayer();
    if (!lp) {
        Chakra::ThrowError(L"Invalid local player");
...
JsValueRef JsLocalPlayerClass::getSelectedBlock(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    auto lp = SDK::ClientInstance::get()->getLocalPlayer();
    if (!lp) {
        Chakra::ThrowError(L"Invalid local player");
    ...
    return JsScript::getThis()->getClass<JsVec3>()->construct(Vec3 {
        static_cast<float>(hr->hitBlock.x), static_cast<float>(hr->hitBlock.y), static_cast<float>(hr->hitBlock.z) });
}

JsValueRef JsLocalPlayerClass::setMovementState(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
...
JsValueRef JsLocalPlayerClass::getMovementState(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    auto lp = SDK::ClientInstance::get()->getLocalPlayer();
    if (!lp) {
        Chakra::ThrowError(L"Invalid local player");
...
JsValueRef JsLocalPlayerClass::turn(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                    unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 2)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsObject } })) return JS_INVALID_REFERENCE;
