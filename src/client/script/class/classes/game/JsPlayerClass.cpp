...
JsValueRef JsPlayerClass::playerGetName(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef JsPlayerClass::playerGetXboxUserID(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
    JsEntity* ent = nullptr;
    JS::JsGetExternalData(arguments[0], reinterpret_cast<void**>(&ent));
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
...
JsValueRef JsPlayerClass::playerGetSelectedItem(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    JsEntity* ent = nullptr;
    JS::JsGetExternalData(arguments[0], reinterpret_cast<void**>(&ent));
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
        if (ent->level != JsEntity::AccessLevel::Restricted) {
            ...
            return JsScript::getThis()->getClass<JsItemStack>()->construct(supp->inventory->getItem(supp->selectedSlot),
                                                                           false);
        } else {
...
JsValueRef JsPlayerClass::playerGetSelectedSlot(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    JsEntity* ent = nullptr;
    JS::JsGetExternalData(arguments[0], reinterpret_cast<void**>(&ent));
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
...
JsValueRef JsPlayerClass::playerGetInventorySlot(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                 unsigned short argCount, void* callbackState) {
    ...
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
        if (ent->level != JsEntity::AccessLevel::Restricted) {
            ...
            return JsScript::getThis()->getClass<JsItemStack>()->construct(
                supp->inventory->getItem(Chakra::GetInt(arguments[1])), false);
        } else {
