...
protected:
    ...
    static JsValueRef CALLBACK toStringCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
        return Chakra::MakeString(L"[object " + std::wstring(reinterpret_cast<JsTextureClass*>(callbackState)->name) +
                                  L"]");
    }

    static JsValueRef CALLBACK loadCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    ...
    static JsValueRef CALLBACK reloadCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
        auto thi = reinterpret_cast<JsTextureClass*>(callbackState);
    ...
    static JsValueRef CALLBACK disposeCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
...
public:
    ...
    JsTextureClass(class JsScript* owner)
        : JsWrapperClass(owner, class_name) {
        createConstructor(jsConstructor, this);
    }

    JsValueRef construct(JsTexture* tex, bool finalize = true) { return __super::construct(tex, finalize); }
