...
JsValueRef JsModuleClass::moduleIsEnabled(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef JsModuleClass::moduleSetEnabled(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                           unsigned short argCount, void* callbackState) {
...
JsValueRef JsModuleClass::moduleIsBlocked(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                          unsigned short argCount, void* callbackState) {
    JsModule* mod = nullptr;
    JS::JsGetExternalData(arguments[0], reinterpret_cast<void**>(&mod));
    if (mod) {
...
JsValueRef JsModuleClass::moduleSetOnEvent(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                           unsigned short argCount, void* callbackState) {
...
JsValueRef JsModuleClass::moduleGetSettings(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    JsModule* mod = nullptr;
    JS::JsGetExternalData(arguments[0], reinterpret_cast<void**>(&mod));
    if (mod) {
...
JsValueRef JsModuleClass::moduleAddBoolSetting(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
    ...
    *set->value = BoolValue(Chakra::GetBool(arguments[4]));
    ...
    return setClass->construct(
        set.get(), false /*do not destroy the setting once it goes out of scope, as module manager will handle that*/);
}

JsValueRef JsModuleClass::moduleAddNumberSetting(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                 unsigned short argCount, void* callbackState) {
...
JsValueRef JsModuleClass::moduleAddKeySetting(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 5)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString },
                                    { arguments[2], JsString },
...
JsValueRef JsModuleClass::moduleAddTextSetting(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 5)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString },
                                    { arguments[2], JsString },
...
JsValueRef JsModuleClass::moduleAddColorSetting(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 5)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsString },
                                    { arguments[2], JsString },
...
JsValueRef JsModuleClass::moduleAddEnumSetting(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
