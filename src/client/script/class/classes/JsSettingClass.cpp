...
JsValueRef JsSettingClass::getValueCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef JsSettingClass::setValueCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    auto thi = reinterpret_cast<JsSettingClass*>(callbackState);
    auto set = Get(arguments[0]);
...
JsValueRef JsSettingClass::setCondition(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                        unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 2, true, true)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            { { arguments[1], JsString }, { Chakra::TryGet(arguments, argCount, 2), JsBoolean, true } }))
        return JS_INVALID_REFERENCE;
    ...
    set->condition = Setting::Condition(util::WStrToStr(Chakra::GetString(arguments[1])), Setting::Condition::EQUALS,
