...
#define GET_RESOURCE(x)                                        \
    ::Resource {                                               \
        &_binary_assets_##x##_start, &_binary_assets_##x##_end \
    }
...
struct Resource {
    ...
    [[nodiscard]] std::span<const std::byte> bytes() const {
        return { reinterpret_cast<const std::byte*>(this->begin()), reinterpret_cast<const std::byte*>(this->end()) };
    }
