...
namespace {
	...
	bool fileHasContent(std::filesystem::path const& path) {
		std::error_code ec;
	...
	bool writeRgbaPng(std::filesystem::path const& path, std::vector<uint8_t> const& rgba, uint32_t width, uint32_t height) {
		...
		if (!encoded || !fileHasContent(tempPath)) {
			std::filesystem::remove(tempPath, ec);
			return false;
		}
	...
	std::string getCachedPlayerHeadTexturePath(SDK::SerializedSkinRef const& skin) {
		...
		if (auto found = cachedPaths.find(hash); found != cachedPaths.end()) {
			if (fileHasContent(found->second)) {
				return found->second;
			}
...
std::string WAILA::getPlayerFaceTexturePath(SDK::Player* player) {
	...
	if (auto playerSkin = player->getSkin()) {
		auto texturePath = getCachedPlayerHeadTexturePath(*playerSkin);
	...
	for (auto& entryPair : *playerList) {
		...
		return getCachedPlayerHeadTexturePath(entry.skin);
	}
