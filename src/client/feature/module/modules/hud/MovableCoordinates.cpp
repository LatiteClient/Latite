...
MovableCoordinates::MovableCoordinates()
    : HUDModule("MovableCoordinates", LocalizeString::get("client.hudmodule.movableCoordinates.name"),
                LocalizeString::get("client.hudmodule.movableCoordinates.desc"), HUD, 0, false) {
    ...
    if (this->isActive() && this->isEnabled()) {
        if (ev.getScreenView()->visualTree->rootControl->name == "hud_screen") {
            this->vanillaCoordinates =
                ev.getScreenView()->visualTree->rootControl->findFirstDescendantWithName("player_position");
