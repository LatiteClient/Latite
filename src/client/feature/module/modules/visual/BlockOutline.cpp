...
BlockOutline::BlockOutline()
    : Module("BlockOutline", LocalizeString::get("client.module.blockOutline.name"),
...
void BlockOutline::onOutlineSelection(Event& evG) {
    ...
    if (!std::get<BoolValue>(outlineVanilla)) ev.setCancelled();

    auto hitResult = SDK::ClientInstance::get()->minecraft->getLevel()->getHitResult();

    if (hitResult->hitType != SDK::HitType::BLOCK) return;
    ...
    MCDrawUtil3D dc { SDK::ClientInstance::get()->levelRenderer, SDK::ScreenContext::instance3d,
                      std::get<BoolValue>(renderThrough)
                          ...
                          : (std::get<BoolValue>(transparent) ? SDK::MaterialPtr::getSelectionOverlayMaterial()
    ...
    if (std::get<BoolValue>(overlay)) {
        ...
        auto drawUp = [&]() {
            dc.fillQuad({ bp.x, bp.y + 1.f, bp.z }, { bp.x + 1.f, bp.y + 1.f, bp.z },
                        { bp.x + 1.f, bp.y + 1.f, bp.z + 1.f }, { bp.x, bp.y + 1.f, bp.z + 1.f }, overlayCol);
        };

        auto drawDown = [&]() {
            dc.fillQuad(bp, { bp.x + 1.f, bp.y, bp.z }, { bp.x + 1.f, bp.y, bp.z + 1.f }, { bp.x, bp.y, bp.z + 1.f },
                        overlayCol);
        };

        auto drawEast = [&]() {
            dc.fillQuad({ bp.x + 1.f, bp.y, bp.z }, { bp.x + 1.f, bp.y + 1.f, bp.z },
                        { bp.x + 1.f, bp.y + 1.f, bp.z + 1.f }, { bp.x + 1.f, bp.y, bp.z + 1.f }, overlayCol);
        };

        auto drawWest = [&]() {
            dc.fillQuad({ bp.x, bp.y, bp.z }, { bp.x, bp.y + 1.f, bp.z }, { bp.x, bp.y + 1.f, bp.z + 1.f },
                        { bp.x, bp.y, bp.z + 1.f }, overlayCol);
        };

        auto drawSouth = [&]() {
            dc.fillQuad({ bp.x, bp.y, bp.z + 1.f }, { bp.x, bp.y + 1.f, bp.z + 1.f },
                        { bp.x + 1.f, bp.y + 1.f, bp.z + 1.f }, { bp.x + 1.f, bp.y, bp.z + 1.f }, overlayCol);
        };

        auto drawNorth = [&]() {
            dc.fillQuad({ bp.x, bp.y, bp.z }, { bp.x, bp.y + 1.f, bp.z }, { bp.x + 1.f, bp.y + 1.f, bp.z },
                        { bp.x + 1.f, bp.y, bp.z }, overlayCol);
        };
