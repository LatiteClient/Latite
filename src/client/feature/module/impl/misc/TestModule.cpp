...
void TestModule::render(DXContext& ctx, bool isDefault, bool inEditor) {
	...
	ctx.drawText({ 0.f, 0.f, rect.getWidth(), rect.getHeight() }, util::StrToWStr(std::format("Input Delay: {:.2f} MS", delayPx)), d2d::Colors::WHITE, Renderer::FontSelection::SegoeLight, 20.f, DWRITE_TEXT_ALIGNMENT_CENTER, DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
}
